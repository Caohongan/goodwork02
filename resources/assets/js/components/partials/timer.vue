...
<div v-if="currentComponent === 'timer'">
  <div class="fixed top-0 text-lg mx-auto mt-16 md:mt-32 max-w-lg z-40 inset-x-0">
    <div class="flex flex-col bg-white shadow-lg relative rounded">
      ...
      <div class="px-4 py-8">
        <button @click="startCountdown" class="px-3 py-2 mx-4 rounded border border-teal-500 font-semibold text-teal-700 start" type="button">Start</button>
        <button @click="pauseCountdown" class="px-3 py-2 mx-4 rounded border" type="button">Pause</button>
        <button @click="resetCountdown" class="px-3 py-2 mx-4 rounded border" type="button">Reset</button>
        <button @click="activateBreak" class="px-3 py-2 mx-4 rounded border" type="button">Break</button>
      </div>
